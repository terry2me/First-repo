...
/** 🚀 Web Worker를 이용한 병렬 최적화 엔진 (원본 복구) */
async function _runOptimizationParallel(codes, packed, simParams, watchData, onProgress) {
  ...
  const workerPromises = chunks.map((chunk, i) => {
    ...
    return new Promise((resolve) => {
      const worker = new Worker('js/fa_worker.js');
      const chunkData = {};
      chunk.forEach(c => {
        if (watchData[c]) chunkData[c] = { candlesWithBB: watchData[c].candlesWithBB };
      });
      ...
      worker.postMessage({ codes: chunk, watchData: chunkData, packed: workerPacked, simParams: simParams });
    });
  });
...
/** 🚀 [WFA] 전진 분석 전용 최적화 엔진 (슬라이딩 윈도우 루프) */
async function runOptimization(codes) {
  ...
  const trainRatio = Number(document.getElementById('faTrainRatio')?.value || 50);
  ...
  SimState.holdDaysActive = false; SimState.targetProfitActive = false; SimState.bbFilterThreshold = 0;
  SimState.optResults = {};
  SimState.simResults = {}; // 결과 초기화
  
  const packed = _packScenarios();
  const yieldTo = () => new Promise(r => setTimeout(r, 0));

  // 2단계: WFA 누적 데이터 초기화 (단리/복리/BH 모두 추적)
  // 2단계: WFA 누적 데이터 초기화
  const wfaResults = {};
  codes.forEach(c => {
    wfaResults[c] = {
      wfaTimeline: {}, // OOS 구간의 최적 지표를 타임라인에 저장
      lastComboForSTB: null, // 이전 달 콤보 값 임시 저장
      comboChangeCount: 0,
      isWinCount: 0,
      isPnlSum: 0, isCount: 0
    };
  });
  ...
  if (windows.length === 0) {
     ...
     btn.disabled = false; btn.textContent = '전진분석';
     return;
  }
  ...
  for (let wIdx = 0; wIdx < windows.length; wIdx++) {
    ...
    const pct = Math.floor((wIdx / windows.length) * 100);
    btn.textContent = `WFA 진행중 (${wIdx + 1}/${windows.length}) ${pct}%`;
    btn.style.background = `linear-gradient(90deg, #9333ea ${pct}%, #7e22ce ${pct}%)`;
    await yieldTo();

    const currentTrainData = {};
    codes.forEach(c => {
      const full = SimState.watchData[c]?.candlesWithBB || [];
      const slice = full.slice(win.trainStart, win.trainEnd);
      if (slice.length > 0) currentTrainData[c] = { ...SimState.watchData[c], candlesWithBB: slice };
    });

    const bests = await _runOptimizationParallel(codes, packed, simParams, currentTrainData, null);

    // 각 종목별 베스트 콤보를 검증 기간에 적용
    for (const code of codes) {
      const b = bests[code];
      if (!b || b.idx === -1) continue;
      
      const bestCombo = packed.combos[b.idx];
      const fullCandles = SimState.watchData[code]?.candlesWithBB || [];
      ...
      if (wfaResults[code].lastComboForSTB) {
        const prev = wfaResults[code].lastComboForSTB;
        const cur = bestCombo;
        const isSame = (
          prev.bbFilterActive === cur.bbFilterActive && prev.bbReq === cur.bbReq && prev.bbTrackingSellActive === cur.bbTrackingSellActive &&
          prev.rsiFilterActive === cur.rsiFilterActive && prev.rsiReq === cur.rsiReq && prev.sellRSIActive === cur.sellRSIActive &&
          prev.stochFilterActive === cur.stochFilterActive && prev.stochReq === cur.stochReq && prev.sellSTOCHActive === cur.sellSTOCHActive &&
          prev.eomFilterActive === cur.eomFilterActive && prev.eomReq === cur.eomReq && prev.sellEOMActive === cur.sellEOMActive &&
          prev.macdFilterActive === cur.macdFilterActive && prev.macdReq === cur.macdReq && prev.sellMACDActive === cur.sellMACDActive &&
          prev.mfiFilterActive === cur.mfiFilterActive && prev.mfiReq === cur.mfiReq && prev.sellMFIActive === cur.sellMFIActive
        );
        if (!isSame) wfaResults[code].comboChangeCount++;
      }
      wfaResults[code].lastComboForSTB = bestCombo;

      const backup = { ...SimState };
      Object.assign(SimState, bestCombo);

      const isRes = _calculateTotalSim({ ...SimState.watchData[code], candlesWithBB: fullCandles.slice(win.trainStart, win.trainEnd) });
      wfaResults[code].isPnlSum += (isRes?.pnl || 0);
  ...
  for (const code of codes) {
    ...
    if (Object.keys(res.wfaTimeline).length === 0) continue;
    found = true;

    // 🚀 [정상화] 타임라인 기반 단일 시뮬레이션으로 '연속성' 확보
    const totalResult = _calculateTotalSim(SimState.watchData[code], res.wfaTimeline);
    
    // 이제 totalResult.trades는 구간 경계에서 잘리지 않은 '완전한 단일 거래'들입니다.
    const finalPnl = (SimState.pnlType === 'compound' ? totalResult.pnl : totalResult.simplePnl);
    
    totalResult.isPnl = (res.isPnlSum / res.isCount);
  ...
  if (found) {
    const firstCode = codes.find(c => wfaResults[c] && wfaResults[c].isCount > 0);
    const best = SimState.optResults[firstCode];
    Object.assign(SimState, best);
    syncUIToState(best); 
    SimState.simDirty = false;
    renderList(); 
    showStockPreview(firstCode);
    showToast(`전진(WFA) 분석 완료!`, 'success');
  }
  btn.textContent = '전진분석'; btn.style.background = ''; btn.disabled = false;
...
async function showStockPreview(code) {
  ...
  SimState.activeCode = code;
  SimState.selectedTradeIndex = -1; // 종목 변경 시 선택 인덱스 초기화
  
  // 🚀 전진분석용 결과(WFA)가 이미 있다면 이력 패널 활성화 및 UI 연동
  const wfaRes = SimState.simResults[code];
  if (wfaRes && wfaRes.trades) {
    SimState.historyActive = true; 
    if (SimState.optResults[code]) {
      syncUIToState(SimState.optResults[code]);
      SimState.simDirty = false;
    }
  } else {
    SimState.historyActive = false;
  }

  // 🚀 원본 메커니즘 100% 활용: previewCode 설정 후 renderPreview 실행
  if (SimState.previewInterval === SimState.listInterval) {
    SimState.previewCode = code;
    SimState.previewData = cached;
    
    // 차트 도화지를 먼저 그린 뒤 (내부에서 simResults를 찾아 마커까지 그림)
...
function _calculateTotalSim(data, wfaParamsMap = null) {
  ...
  for (let i = startIdx; i <= endThreshold; i++) {
    ...
    const anyFilter = params.bbFilterActive || params.eomFilterActive || params.rsiFilterActive || params.stochFilterActive || params.macdFilterActive || params.mfiFilterActive;
    if (!anyFilter) continue;

    const isBuyToday = params.simBuyTiming === 'today';
    const isSellToday = params.simSellTiming === 'today';

    // 1. 오늘의 시그널 조건 체크 (BB + EOM + RSI 교집합)
    let bbOk = !params.bbFilterActive;
    if (params.bbFilterActive) {
      for (let k = 0; k < win; k++) {
        const curIdx = i - k;
        if (curIdx < 0) break;
    ...
    if (allReqMet && optConditionMet) {
      ...
      for (let j = 0; j < maxWatchDays; j++) {
        ...
        if (!candles[checkIdx]) {
          ...
          exitPrice = candles[actualExitIdx]?.[params.bbSellPriceType] ?? candles[actualExitIdx]?.close ?? 0;
          exitDate = null;
          exitReason = '진행중';
          hitTarget = true;
          break;
        ...
        const sPrice = candles[checkIdx]?.[params.bbSellPriceType] ?? candles[checkIdx]?.close ?? 0;
        const raisePrice = candles[checkIdx]?.close ?? 0; // 마지노선 상승 판별은 종가 기준으로 고정 (민감도 유지)

        if (params.bbTrackingSellActive && curBB.middle !== null) {
        ...
        if (params.bbTrackingSellActive && currentMajinosen !== null) {
        ...
        if (!hitTarget && params.targetProfitActive) {
          const targetPrice = buyPrice * (1 + params.simTargetProfit / 100);
        ...
        // 3) 기술적 지표 매도 신호
        if (!hitTarget && params.sellEOMActive && candles[checkIdx].eomCross === 'SELL') {
          exitReason = 'EOM매도';
          actualExitIdx = isSellToday ? checkIdx : checkIdx + 1;
          hitTarget = true;
        }
        if (!hitTarget && params.sellRSIActive && candles[checkIdx].rsiSignal === 'SELL') {
          exitReason = 'RSI매도';
          actualExitIdx = isSellToday ? checkIdx : checkIdx + 1;
          hitTarget = true;
        }
        if (!hitTarget && params.sellSTOCHActive && candles[checkIdx].stochSignal === 'SELL') {
          exitReason = 'ST매도';
          actualExitIdx = isSellToday ? checkIdx : checkIdx + 1;
          hitTarget = true;
        }
        if (!hitTarget && params.sellMACDActive && candles[checkIdx].macdCross === 'SELL') {
          exitReason = 'MACD매도';
          actualExitIdx = isSellToday ? checkIdx : checkIdx + 1;
          hitTarget = true;
        }
        if (!hitTarget && params.sellMFIActive && candles[checkIdx].mfiSignal === 'SELL') {
          exitReason = 'MFI매도';
          actualExitIdx = isSellToday ? checkIdx : checkIdx + 1;
          hitTarget = true;
        }
        ...
        // 4) 보유기간 만료 (Option)
        if (!hitTarget && params.holdDaysActive) {
          if (j >= params.simHoldingDays - 1) {
            exitReason = '기간만료';
            actualExitIdx = isSellToday ? checkIdx : checkIdx + 1;
            hitTarget = true;
          }
      ...
      const reasons = [];
      if (params.bbFilterActive && bbOk) reasons.push('BB');
      if (params.eomFilterActive && eomOk) reasons.push('EOM');
      if (params.rsiFilterActive && rsiOk) reasons.push('RSI');
      if (params.stochFilterActive && stochOk) reasons.push('ST');
      if (params.macdFilterActive && macdOk) reasons.push('MACD');
      if (params.mfiFilterActive && mfiOk) reasons.push('MFI');
      // 모두 false인 경우 (opt 조건 충족 등)는 활성 필터를 폴백으로 표시
      if (reasons.length === 0) {
        if (params.bbFilterActive) reasons.push('BB');
        if (params.eomFilterActive) reasons.push('EOM');
        if (params.rsiFilterActive) reasons.push('RSI');
        if (params.stochFilterActive) reasons.push('ST');
        if (params.macdFilterActive) reasons.push('MACD');
        if (params.mfiFilterActive) reasons.push('MFI');
      }
  ...
  trades.forEach(t => {
    ...
    const exitPriceFinal = candles[end]?.[originalSimState.bbSellPriceType] || candles[end]?.close || 0;
    if (basePrice > 0) {
...
function buildListItem(stock, data, rowNum, isPinned) {
  ...
  if (res) {
    ...
    // 신규 지표 포맷팅
    const wfeVal = res.wfeScore || 0;
    const wfeClass = wfeVal >= 50 ? 'up log-in' : wfeVal < 20 ? 'down log-in' : '';
    const stbVal = res.stbScore || 0;
    const stbStr = `${Math.trunc(stbVal)}%`;
    ...
    item.innerHTML = `
      ...
      <div class="col-item col-sim-wfe ${wfeClass}">${Math.trunc(wfeVal)}%</div>
      <div class="col-item col-sim-stb">${stbStr}</div>`;
  } else {
    // 결과 없을 때 기본값
    item.innerHTML = `
      <div class="col-idx">${rowNum || ''}</div>
      ...
      <div class="col-item col-sim-wfe">--</div>
      <div class="col-item col-sim-stb">--</div>`;
  }
  ...
  item.addEventListener('click', e => {
    ...
    if (e.ctrlKey) {
      ...
      const hasWfaResult = SimState.simResults[code] && SimState.simResults[code].trades;

      if (hasWfaResult) {
        // 이미 전진분석 결과가 있다면 재계산 없이 바로 출력 (동기화 로직만 수행)
        if (SimState.optResults[code]) {
          syncUIToState(SimState.optResults[code]);
          SimState.simDirty = false; // 재계산 필요 없음
        }
        showStockPreview(code);
        highlightActiveRow(code);
        // _markSimDirty(true) 호출을 건너뜀 (값이 튀는 현상 방어)
      } else {
        // 전진분석 결과가 없는 일반 상태일 때만 원본 로직 수행
        if (SimState.optResults && SimState.optResults[code]) {
          const combo = SimState.optResults[code];
        ...
        showStockPreview(code);
        highlightActiveRow(code);
        if (SimState.optResults && SimState.optResults[code]) renderList();
      }
    }
  });
  return item;
  ...
  return [...list].sort((a, b) => {
    ...
    switch (col) {
      ...
      case 'simRMD': 
        ...
        break;
      case 'simWFE': va = ra?.wfeScore ?? -Infinity; vb = rb?.wfeScore ?? -Infinity; break;
      case 'simSTB': va = ra?.stbScore ?? -Infinity; vb = rb?.stbScore ?? -Infinity; break;
...
function initColResizers() {
  ...
  document.querySelectorAll('.col-resizer').forEach(h => {
    ...
    h.addEventListener('mousedown', e => {
      ...
      // max-content일 수 있으므로 실제 렌더링 너비를 가져옴
      sw = h.parentElement.offsetWidth || 80;
      sx = e.clientX;
